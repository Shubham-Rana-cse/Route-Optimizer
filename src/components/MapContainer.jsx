...
const MapContainer = () => {
  ...
  const getRoutePaths = useCallback(() => {
    if (!optimizedRoute || !optimizedRoute.path.length) {
      return { mainPath: [], returnPath: [] }
    }
  ...
  return (
    ...
    >
      ...
      {mainPath.length > 0 && (
        <Polyline
          positions={mainPath}
          pathOptions={{ color: '#1E40AF', weight: 4, opacity: 0.7 }}
        />
      )}
      ...
      {returnPath.length > 0 && (
        <Polyline
          ...
          pathOptions={{ color: '#780404', weight: 4, opacity: 0.7 }}
        />
      )}
    </LeafletMap>
  )
